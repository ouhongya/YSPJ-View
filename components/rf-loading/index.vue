		...
		<view class="mask" v-show="isActive" :class="{ 'full-screen': isFullScreen }" :style="{ backgroundColor }">
			<view class="spinner" :style="{ transform: `translate(-50%, -${translateY}%)` }">
				<slot>
					...
					<longrect v-if="styleLoadingType === 'longrect'" :color="themeColor.color" :size="size" />
					<note v-if="styleLoadingType === 'note'" :color="themeColor.color" :size="size" />
					<blockract v-if="styleLoadingType === 'blockract'" :color="themeColor.color" :size="size" />
				</slot>
...
<script>
	...
	import longrect from './loaders/longrect.vue';
	import note from './loaders/note.vue'
	import blockract from './loaders/blockract.vue'
	// ==========在这里选一个您要的其他去掉也行==========
