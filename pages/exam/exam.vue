<template>
	<view class="exam">
		<a-head :showBack="false" bgColor="#4abdb4" :text="name"></a-head>
		<view class="header" id="header" :class="ipadflag ? 'ipadw' : ''">
			<view class="header-left">第{{ currentIndex + 1 }}/{{ questionList.length }} 题</view>
			<view class="header-right"><view class="question" @click="question()">答题卡</view></view>
		</view>
		<view id="subHeader" :class="ipadflag ? 'ipads' : ''">
			<view class="sub-header" v-if="questionList.length > 0">
				<view class="left" v-if="questionList[currentIndex]">
					...
					<view class="sub-header-number">
						({{ questionList[currentIndex].score }}分/题
						<text>共{{ questionList[currentIndex].size }}题</text>
						)
					</view>
				</view>
				<view class="sub-right">
					<u-icon name="hourglass-half-fill" color="#19be6b" size="32"></u-icon>
					<u-count-down ref="uCountDown" :timestamp="timestamp" :autoplay="false" :show-days="false" @change="change" @end="end"></u-count-down>
				</view>
			</view>
		</view>
		<swiper
					...
					<scroll-view scroll-y :style="{ height: swiperHeight }">
						...
						<view class="content-solutions" v-if="item.type == 3">
							<template v-for="subItem in item.optionList">
								<view class="content-solutions-item" :class="item.userAnswer == subItem.id ? 'selectchoose' : ''">
									<view
										:class="item.userAnswer == subItem.id ? 'content-solutions-item-select' : ''"
										class="content-solutions-item-check-content"
										@click="chooseSolution(item, subItem)"
									>
										{{ subItem.content }}
									</view>
								</view>
						...
						<view class="content-solutions" v-if="item.type == 1">
							<template v-for="subItem in item.optionList">
								<view class="content-solutions-item" @click="chooseSolution(item, subItem)" :class="item.userAnswer == subItem.id ? 'select' : ''">
						...
						<view class="content-solutions" v-if="item.type == 2">
							<template v-for="subItem in item.optionList">
								<view class="content-solutions-item" @click="chooseMutiSolution(item, subItem)" :class="item.userAnswer.indexOf(subItem.id) > -1 ? 'select' : ''">
									<view class="content-solutions-item-single">{{ subItem.id }}</view>
		...
		<view class="footer" id="footer" :class="ipadflag ? 'ipadw' : ''">
			...
			<view class="footer-card"><u-button type="success" @click="handleSubmit">提交试卷</u-button></view>
		...
		<!-- 确认提交 -->
		<u-modal v-model="showSubmit" @confirm="submitconfirm" :async-close="true" :show-cancel-button="true" :confirm-style="{ color: '#19be6b' }" :show-title="false">
			<view class="slot-content">
				<view class="model">
					<view class="stop">您确定要提交吗？</view>
					<view class="tips" v-if="respondence !== ''">
						题目
						<text>{{ respondence }}</text>
						题尚未作答！
					</view>
		...
		>
			<view class="slot-content">
				<view class="model">
					<view class="start">{{ questionList[0].remarks }}</view>
					<!-- <view class="start">考试过程中请保持网络连接</view> -->
				</view>
			</view>
		</u-modal>
		<!-- 答题卡 -->
				...
				<scroll-view scroll-y="true" class="sroll">
					<view class="content">
						<view v-for="(item, index) in questionList" :key="index" class="option">
							<view class="circle" :class="item.userAnswer == '' ? '' : 'blue'" @click="handleJumpSwiper(index)">{{ index + 1 }}</view>
						</view>
					</view>
				</scroll-view>
			</view>
		...
		<u-modal v-model="resultShow" :show-confirm-button="false" title="考试结果">
			<view class="slot-content">
				<view class="model ">
					<view class="bgmodel">
						<view class="result">考试得分：{{ result.score }}</view>
						<view class="result">合格分值：{{ result.rightScore }}</view>
						<!-- <view class="result">考试结果：<text class="green">合格</text></view> -->
						<view class="result">
							考试结果：
							<text class="red">{{ result.back }}</text>
						</view>
					</view>
					<view class="action">
						<u-button type="success" size="medium " @click="finish()">完成考试</u-button>
						<u-button type="warning" size="medium " @click="again()" v-if="result.isFlag == 1">重考</u-button>
					</view>
				</view>
			</view>
		</u-modal>
...
<script>
import { startExam, startUserExam, startUserAnswer, submitTestPaper, questionAgain,continueExamination,answers } from '@/api/exam/testbase.js';
export default {
	data() {
		return {
			examUserId:null,
	...
	onLoad(options) {
		...
		this.isFlag = this.options.isFag;
		this.examUserId = this.options.examUserId;
		if (this.options.button != '嘿嘿') {
			this.button = this.options.button;
		}
		if (this.isFlag == 1) {
			this.startExam(options.id);
		} else {
			this.startExam(options.id);
		}
	},
	...
	methods: {
		//点击答题
		startUserAnswer(item, Flag) {
			let answerStr = 0;
			item.answer == item.userAnswer ? (answerStr = 0) : (answerStr = 1);
			startUserAnswer(item.exam_id, item.exam_user_topic_id, item.userAnswer, answerStr).then(res => {
				if (Flag) {
					this.currentIndex += 1;
				}
				if (res == 'SUCCESS') {
				...
				if (res == 'EXAM_END') {
					console.log('单场考试时间结束');
					this.answers(this.questionList[0].exam_id, this.questionList[0].exam_user_id);
				}
				if (res == 'EXAM_END_TIME') {
					console.log('考试时间已结束');
					this.answers(this.questionList[0].exam_id, this.questionList[0].exam_user_id);
				}
				if (res == 'EXAM_SUSPEND') {
		...
		answers(exam_id, exam_user_id){
			answers(exam_id, exam_user_id).then(res=>{
				this.result.score = res.score;
				this.result.rightScore = res.rightScore;
				this.result.back = res.status == 0 ? '合格' : '不合格';
				this.result.isFlag = res.isFlag;
		...
		//拉取试题列表
		startExam(id) {
			startExam(id).then(res => {
				this.questionList = null;
				this.timestamp = res[0].examTimeNew;
				res = res.sort((a, b) => {
		...
		tipsconfirm() {
			if(this.isFlag==1){
				if (this.button == '开始考试') {
					startUserExam(this.questionList[0].exam_user_id, this.questionList[0].examTimeNew, this.questionList[0].pauseTime).then(res => {
						this.timestamp = this.questionList[0].examTimeNew;
						this.showTips = false;
						this.$refs.uCountDown.start();
					});
				} else {
					continueExamination(this.questionList[0].exam_user_id, this.questionList[0].examTimeNew, this.questionList[0].pauseTime).then(res => {
						this.timestamp = this.questionList[0].examTimeNew;
						this.showTips = false;
						this.$refs.uCountDown.start();
					});
				...
				questionAgain(this.id,this.examUserId).then(res=>{
					startExam(this.id).then(res => {
						this.questionList = null;
						this.timestamp = res[0].examTimeNew;
						res = res.sort((a, b) => {
							return parseInt(a.type) - parseInt(b.type);
						...
						this.currentIndex = 0;
						this.$forceUpdate();
						startUserExam(this.questionList[0].exam_user_id, this.questionList[0].examTime, this.questionList[0].pauseTime).then(res => {
							this.timestamp = this.questionList[0].examTimeNew;
							this.showTips = false;
							this.$refs.uCountDown.start();
						});
		...
		chooseSolution(item, subItem) {
			//exam_id, exam_user_id, userAnswer,right
			item.userAnswer = subItem.id;
			this.startUserAnswer(item, true);
		},
		// 选择答案（多选）
		chooseMutiSolution(item, subItem) {
			...
			let noFormString = splitArray
				.sort()
				.toString()
				.replace(/,/g, '');
			item.userAnswer = noFormString;
			this.startUserAnswer(item, false);
		},
		//提交
		handleSubmit() {
			this.respondence = '';
		...
		submitconfirm() {
			...
			this.submitTestPaper(this.questionList[0].exam_id, this.questionList[0].exam_user_id, 22);
		},
		//提交考试
		submitTestPaper(exam_id, exam_user_id, type) {
			submitTestPaper(exam_id, exam_user_id, type).then(res => {
				this.result.score = res.score;
				this.result.rightScore = res.rightScore;
				this.result.back = res.status == 0 ? '合格' : '不合格';
				this.result.isFlag = res.isFlag;
		...
		//重考
		again() {
			questionAgain(this.questionList[0].exam_id, this.questionList[0].exam_user_id).then(res => {
				this.options.button = '嘿嘿';
				this.startExam(res);
