<template>
	<view class="main">
		<a-head :text="head" bgColor="#4abdb4"></a-head>
		<view class="" v-if="isFlag==1">
								...
								<view class="range">
									<u-cell-group>
										<u-cell-item title="所属题库" :title-style="labelStyle" :arrow="true"
											arrow-direction="right" :value="item.belong" @click="toBelong()">
										</u-cell-item>
									</u-cell-group>
								</view>
								...
								<u-form-item label="正确答案">
									...
									<view class="" v-if="item.type == 1">
										<u-radio-group v-model="item.answer" active-color="#4abdb4"
											...
											<u-radio v-for="(j, index3) in singleList" :key="index3" :name="j.name"
									...
									<view class="" v-if="item.type == 2">
										<u-checkbox-group @change="checkboxGroupChange($event, index)"
											active-color="#4abdb4">
											<u-checkbox v-model="j.checked" v-for="(j, index3) in checkboxList"
									...
									<view class="" v-if="item.type == 3">
										<u-radio-group v-model="item.answer" active-color="#4abdb4"
											...
											<u-radio v-for="(j, index3) in judgeList" :key="index3" :name="j.name"
												:disabled="j.disabled">{{ j.name }}</u-radio>
										</u-radio-group>
									</view>
								</u-form-item>
				...
				<swiper-item v-for="(item, index) in examlist" :key="index">
					<scroll-view scroll-y="true" class="swiper">
						<view class="content">
							<view class="other">
								<view class="list">
									所属题库：<text>{{item.typename}}</text>
								</view>
								<view class="list">
									题目类型：<text>{{item.belong}}</text>
								</view>
								<view class="list">
									题目标题：<text>{{item.title}}</text>
								</view>
								<view class="list">
									选项内容:
								</view>

								<view class="option-part" v-for="(i, index1) in item.optionList" :key="index1">
									<view class="option"><text>{{ i.optionId }}</text>{{i.content}}</view>

								</view>
								<view class="list">
									正确答案：<text>{{item.answer}}</text>
								</view>
							</view>
						</view>
					</scroll-view>
		...
		<!-- 所属题库选择 -->
		<u-select v-model="belongShow" mode="single-column" :default-value="belongIndex" :list="belongList"
		...
		methods: {
			...
			save() {
				...
				editOneTopic(data, stemData).then(res => {
					if (res == 'SUCCESS') {
						uni.navigateTo({
							url: './testbase?id=' + [data[0].exam_category_id] + '&type=1'
						});
					} else {
