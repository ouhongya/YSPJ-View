<template>
	<view class="exam">
		...
		<view class="header" id="header" :class="ipadflag ? 'ipadw' : ''">
			...
			<view class="header-right">试题来源：{{questionList[currentIndex].source}}</view>
		</view>
		<view id="subHeader" :class="ipadflag ? 'ipads' : ''">
			<view class="sub-header" v-if="questionList.length > 0">
				...
				<view class="sub-right"><u-button :plain="true" size="mini" type="primary" @click="look(currentIndex)">查看答案</u-button></view>
			</view>
		</view>
		<swiper
			class="content"
					...
					<scroll-view scroll-y :style="{ height: swiperHeight }">
						...
						<view class="content-solutions" v-if="item.type == 3">
							<template v-for="subItem in item.optionList">
								<view :class="item.isReviewed && item.answer.indexOf(subItem.id) > -1 ? 'right' : ''" class="content-solutions-item">
									<view
						...
						<view class="content-solutions" v-if="item.type == 1">
							<template v-for="subItem in item.optionList">
								<view
									:class="item.isReviewed && item.answer.indexOf(subItem.id) > -1 ? 'right' : ''"
									class="content-solutions-item"
									@click="chooseSolution(item, subItem)"
								>
									<view class="content-solutions-item-single" :class="item.userAnswer == subItem.id ? 'select' : ''">{{ subItem.id }}</view>
						...
						<view class="content-solutions" v-if="item.type == 2">
							<template v-for="subItem in item.optionList">
								<view
									:class="item.isReviewed && item.answer.indexOf(subItem.id) > -1 ? 'right' : ''"
									class="content-solutions-item"
									@click="chooseMutiSolution(item, subItem)"
								>
						...
						<view class="content-item-explain" v-if="item.isReviewed">
							...
							<view class="content-item-explain-result" v-if="item.userAnswer">
								你的答案:
								<text :class="item.answer == item.userAnswer ? 'correct' : 'error'">{{ item | rightuserAnswerFilter }}</text>
							</view>
		...
		<view class="footer" id="footer" :class="ipadflag ? 'ipadw' : ''">
			...
			<view class="footer-card"><u-button type="success" @click="end()">结束练习</u-button></view>
...
<script>
import { startExam, startUserExam,startUserAnswer,submitTestPaper,questionAgain } from '@/api/exam/testbase.js';
export default {
	data() {
		return {
			currentIndex: 0,
		...
		startUserAnswer(item){
			...
			startUserAnswer(item.exam_id,item.exam_user_topic_id,item.userAnswer,answerStr).then(res=>{
				...
				if(res=="EXAM_END_TIME"){
					console.log("考试时间已结束")
					this.submitTestPaper(this.questionList[0].exam_id,this.questionList[0].exam_user_id,33)
				}
				if(res=="EXAM_SUSPEND"){
					console.log("考试中止")
				}
