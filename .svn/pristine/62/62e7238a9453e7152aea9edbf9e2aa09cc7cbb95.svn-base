<template>
	<view class="u-upload" v-if="!disabled">
		...
		<!-- 删除弹框 -->
		<u-modal v-model="showdel" @confirm="delconfirm" content="您确认要移除该图片吗？" :show-title="false" :show-cancel-button="true"
...
<script>
	...
	export default {
		...
		methods: {
				...
				chooseFile
					.then(res => {
						...
						res.tempFiles.map((val, index) => {
                            ...
                            let that = this;
								uni.saveFile({
									tempFilePath: val.path,
									success: function(red) {
										var address = red.savedFilePath;
										var img = {
											url: val.path,
											progress: 0,
											image: address,
											title: address.split("/")[2].split(".")[0],
											name: address.split("/")[2].split(".")[0],
											url: address,
											type: 1,
											id: address.split("/")[2].split(".")[0],
											error: false
										}
										lists.push(img)
										that.$emit('on-success', img, "", img);
									}
								});
			...
			delconfirm() {
				if (this.lists[this.delindex].process < 100 && this.lists[this.delindex].process > 0) {
					typeof this.lists[this.delindex].uploadTask != 'undefined' && this.lists[this.delindex].uploadTask.abort();
				}
			    var obj=this.lists[this.delindex];
				this.lists.splice(this.delindex, 1);
				this.$forceUpdate();
				this.$emit('on-remove', this.delindex, this.lists,obj);
				this.showToast('移除成功');
			},
			// 用户通过ref手动的形式，移除一张图片
			remove(index) {
