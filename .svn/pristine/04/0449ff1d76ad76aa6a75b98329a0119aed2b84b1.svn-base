<template>
	<view class="main">
		...
		<view class="part-top">
			...
			<view class="part color2" @click="toBase()"><view class="txt">题库管理</view></view>
			<view class="part color3" @click="toPublish()"><view class="txt">发布考试</view></view>
			<view class="part color4" @click="toPractice()"><view class="txt">考试练习</view></view>
		</view>
		<view class="center">
			<view class="list-center">
				<u-section title="我发布的考试" sub-title="更多" @click="toMore()" class="section" font-size="30"></u-section>
				<uni-swipe-action>
			...
			<view class="list-center">
				<u-section title="我的考试" :right="false" class="section" font-size="30"></u-section>
				<mescroll-uni :top="0" @down="downCallback" @up="upCallback" :up="upOption" @init="mescrollInit" ref="mescrollRef" height="400px">
					<view class="list-my" v-for="(item, index) in list" :key="index">
						<view class="title">{{ item.name }}({{ item.createdTime }})</view>
						<view class="left">
							考试时间：
							<text>{{ $u.timeFormat(item.startTime, 'yyyy-mm-dd') }}</text>
...
export default {
	...
	onLoad(options) {
		if (options.isFlag !== undefined) {
			this.queryExamListRoleLaunch();
		}
	},
	onShow() {
		this.queryExamListRoleLaunch();
	...
	methods: {
		...
		//拉取我考试列表
		queryExamListRoleLaunch() {
			queryExamListRoleLaunch(1, 2).then(res => {
				let data = res.list;
				data.forEach(item => {
					item.show = false;
					if (item.status == 0) {
...
.part-top {
	...
	.color2 {
		background: linear-gradient(to right, #ed6e66, #e63f34);
	}
